...
class ApiClient:
    ...
    def get(self, url: str) -> Any:
        ...
        self._wait()
        try:
            request = Request(full_url, headers={"User-Agent": self.config.user_agent})
            with urlopen(
            ...
            raise ApiConnectionError(
                f"HTTP Error {e.code}: {e.reason}", url=full_url, status_code=e.code
            ) from e
        except URLError as e:
            raise ApiConnectionError(
                f"Connection failed: {e.reason}", url=full_url
            ) from e
        except json.JSONDecodeError as e:
            raise ApiResponseError(f"Invalid JSON response: {e}") from e
        except TimeoutError:
            raise ApiConnectionError(
                f"Request timed out after {self.config.request_timeout}s", url=full_url
            ) from None
